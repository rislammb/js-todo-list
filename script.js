...
const filterTodos = searchStr =>
  getTodos().filter(todo =>
    todo.toLowerCase().includes(searchStr.toLowerCase())
  );
...
const displayTodos = () => {
  ...
  getTodos().forEach(_todo => appenListToDom(buildTodoEl(_todo)));
  initSearchListener();
  initClickListeners();
};
...
const initSearchListener = () => {
  document.getElementById('search-input').addEventListener('keyup', $event => {
    ...
    clearTodoListDisplay();
    filtered.forEach(_todo => appenListToDom(buildTodoEl(_todo)));
    initClickListeners();
  });
};
